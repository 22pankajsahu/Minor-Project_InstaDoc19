...
<?php
	if(isset($_REQUEST['cr_to_id'])){
		$cr_to_id = $_REQUEST['cr_to_id'];
	}
	$docselect = "select firstname,docimage from user inner join doctor on user_id=$cr_to_id where uid=$cr_to_id";
	$docresult = mysqli_query($conn, $docselect);
	$docrow = mysqli_fetch_assoc($docresult);
	
	$crselect = "select cr_from_id from chatrequest where cr_from_id=$user_id";
	$crresult = mysqli_query($conn, $crselect);
	if(mysqli_num_rows($crresult)<=0){
		$crinsert = "insert into chatrequest (cr_from_id,cr_to_id,cr_status,cr_video_link,cr_passcode)values('$user_id','$cr_to_id','0','none','none')";
		mysqli_query($conn, $crinsert);
	}
                ...
                <div class="card">
                    <div class="card-header msg_head">
                        <div class="d-flex bd-highlight">
                            <div class="img_cont">
                                <img src="../<?php echo $docrow['docimage'];?>" class="rounded-circle user_img">
                                <span class="online_icon"></span>
                            </div>
                            <div class="user_info">
                                <span>Dr. <?php echo $docrow['firstname'];?></span>
                            </div>
                            <div class="video_cam" id="vidlink">
                    ...
                    <div class="card-body msg_card_body" id="mychatbox">
                        <!-- Chats-->
                    </div>
                    <div class="card-footer">
                        <div class="input-group">
                            ...
                            <div class="input-group-append">
                                <span class="input-group-text send_btn" id="send"><i class="fa fa-location-arrow"></i></span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        $(document).ready(function () {
			...
			setInterval(function (){
				$.ajax({
					...
					success: function(response4){
						if(response4==0){
							document.getElementById("vidlink").style.display="none";
						}else{
							document.getElementById("vidlink").style.display="block";
							document.getElementById("vidlink").innerHTML=response4;
							clearInterval(this);
						}
					}
				});
			...
			setInterval(function (){
				...
				$.ajax({
					...
					url:"messages.php",
					data:{
						cr_to_id:<?php echo $cr_to_id;?>,
					},
					success: function(response2){
						document.getElementById("mychatbox").innerHTML=response2;
					}
				});
			},1000);
