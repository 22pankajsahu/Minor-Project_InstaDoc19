...
<body>
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-7">
                <h1 class="text-center registration-title"> Upload Disease </h1>
				<h2 class="text-center registration-title">
					<?php
						...
						if(isset($_POST["upload"])){
							...
							if(empty($ds_name) || empty($ds_info) || empty($_FILES["ds_image"]["tmp_name"])){
								echo "All fields are required.";
							}
							else{	
								$fileSize = $_FILES['ds_image']['size'];
								if($fileSize < 300000){
								
									$ds_image = $_POST['ds_image'];
									$target_directory = "../../patient/images/";
									$target_file = $target_directory.basename($_FILES["ds_image"]["name"]); 
									$filetype = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
									$newfilename = uniqid('', true).".".$filetype;
									$path = "images/".$newfilename;
									if(move_uploaded_file($_FILES["ds_image"]["tmp_name"],$target_directory.$newfilename)){
										
										$diseaseentry = "insert into disease (ds_name,ds_info,ds_image,ds_doctor) 
											values ('$ds_name','$ds_info', '$path', '$user_id')";
										if(mysqli_query($conn, $diseaseentry)){
                ...
                <div class="account-wall">
                    <form class="form form-registration" action="" method="post" enctype="multipart/form-data">
                        ...
                        <input type="text" class="form-control" id="ds_name" name="ds_name" placeholder="Title" required>
						
                        <label for="content">Content</label>
                        <textarea rows="3" class="form-control" id="ds_info" name="ds_info" placeholder="Content" required></textarea>

                        <label for="image">Upload Image</label>
                        <input type="file" class="form-control" id="ds_image" name="ds_image" required>
